...
function get_users() {
	...
	$.get( 'http://sws.tailoreddev.co.uk/ajax/get-users.php', function( data ) {
		...
		$.each( data, function ( i, item ) {
			...
			db.transaction( 
				function( tx ) {
					tx.executeSql( query, [], function(tx, rs ) { console.log( 'Inserted user: '+data[i].userName ); }, errorCB ); 
				}
			);
		});
		$('.users-sync .rem-icon').remove();
...
function get_clients() {
	...
	$.get( 'http://sws.tailoreddev.co.uk/ajax/get-clients.php', function( data ) {
		...
		$.each( data, function ( i, item ) {
			...
			db.transaction( 
				function( tx ) {
					tx.executeSql( query, [], function(tx, rs ) { console.log( 'Inserted client: '+data[i].clientName ); }, errorCB ); 
				}, errorCB
			);
		});
		$('.clients-sync .rem-icon').remove();
...
function get_reports() {
	...
	$.get( 'http://sws.tailoreddev.co.uk/ajax/get-reports.php', function( data ) {
		...
		$.each( data, function ( i, item ) {
			...
			db.transaction( 
				function( tx ) {
					tx.executeSql( query, [], function(tx, rs ) { console.log( 'Inserted report: '+data[i].reportID ); }, errorCB ); 
				}, errorCB
			);
		});
	...
	$.get( 'http://sws.tailoreddev.co.uk/ajax/get-obs.php', function( data ) {
		...
		$.each( data, function ( i, item ) {
			...
			db.transaction( 
				function( tx ) {
					tx.executeSql( query, [], function(tx, rs ) { console.log( 'Inserted obs: '+data[i].obsID ); }, errorCB ); 
				}, errorCB
			);
		});
	}, 'json' );
...
function get_contracts() {
	...
	$.get( 'http://sws.tailoreddev.co.uk/ajax/get-contracts.php', function( data ) {
		...
		$.each( data, function ( i, item ) {
			...
			db.transaction( 
				function( tx ) {
					tx.executeSql( query, [], function(tx, rs ) { console.log( 'Inserted contract: '+data[i].contractID ); }, errorCB ); 
				}, errorCB
			);
		});
	}, 'json' );
